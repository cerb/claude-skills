...
- **[Automations/Scripting]** In automation scripting, added a vobject\_parse(string) function for processing vCard (VCF) and iCalendar (.ics) files in automation workflows. The function takes a block of text in VObject format as input, and returns a parent/child data structure. [#318]
...
- **[Automations/Scripting]** In automation scripting, added a new date\_lerp(date\_range,unit,step,limit) function for interpolating the timestamps between two dates with the given `unit` (`minute`, `day`, `week`, `month`) and `step` (1, 5, 10).
