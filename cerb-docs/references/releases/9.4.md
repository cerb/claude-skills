...
```
type: gpg.keyinfo fingerprint: A1B2C3D4E5F6 filter: subkeys format: dictionaries
```
...
```
- icon : key : _icon label : Icon params : image : circle-ok
```
...
```
- card : key : _label label : Label params : icon : image_template : | {% if 'closed' == status %} circle-ok {% elseif 'waiting' == status %} stopwatch {% endif %} bold : true
```
...
```
- text : key : status label : Status params : icon : image_template : | {% if 'closed' == status %} circle-ok {% elseif 'waiting' == status %} stopwatch {% endif %}
```
...
```
type: worklist.records of: comment query: () expand: ["attachments~10:on,custom_"] format: dictionaries
```
...
```
type: worklist.records of: attachment query: (name:*.png on:comment) expand: ["on.comment:_label,author_,custom_"] format: dictionaries
```
...
```
type: worklist.records of: ticket query: (status:open) expand: ["comments~5:author_,target_"] format: dictionaries
```
...
```
type: worklist.records of: ticket query: (status:o) expand: ["links.project_board_column:_label,board_"] format: dictionaries
```

```
type: worklist.records of: ticket query: (status:o) expand: ["links.task~25:_label,comments"] format: dictionaries
```
