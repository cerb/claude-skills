...
When workers reply to tickets, they can use the _"waiting"_ status to indicate that a response from the client is needed before continuing. When a client replies, the ticket status is automatically changed back to _"open"_, and notifications are sent to the owner and watchers.
