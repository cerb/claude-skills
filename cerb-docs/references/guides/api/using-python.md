...
```
redirect_uri = ' http://localhost/ ' request_base_url = ' http://localhost/ ' auth_url = request_base_url + ' oauth/authorize ' token_url = request_base_url + ' oauth/access_token ' extra = { ' client_id ' : ' ... ' , ' client_secret ' : ' ... ' , }
```
...
```
import json from requests_oauthlib import OAuth2Session
```
...
```
def token_saver ( token ): # [TODO] Persist the token somewhere
 pass
```
...
```
def token_loader (): # [TODO] Load the token from somewhere (inverse of token_saver)
 return { ' token_type ' : ' Bearer ' , ' expires_in ' : 300 , ' access_token ' : ' eyJ0... ' , ' refresh_token ' : ' def500... ' , } oauth = OAuth2Session ( client_id = extra [' client_id '], token = token_loader (), auto_refresh_url = token_url , auto_refresh_kwargs = extra , token_updater = token_saver )
```
...
```
scopes = [' api '] oauth = OAuth2Session ( client_id = extra [' client_id '], redirect_uri = redirect_uri , scope = scopes , auto_refresh_url = token_url , auto_refresh_kwargs = extra , token_updater = token_saver ) authorization_url , state = oauth . authorization_url ( auth_url , access_type = " offline " ) print ( ' Visit %s in your browser and authorize access. ' % authorization_url )
```
...
```
authorization_response = input ( " Paste the callback URL from your browser location bar: " ) token = oauth . fetch_token ( token_url , authorization_response = authorization_response , client_secret = extra [' client_secret '] )
```
...
```
import json json . loads ( r . content )
```
