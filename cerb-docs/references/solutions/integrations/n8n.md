...
```
workflow: name: wgm.integrations.n8n version: 2025-03-29T04:45:37Z description: A sample integration with n8n workflows website: https://cerb.ai/resources/workflows/ requirements: cerb_version: >=11.0 <11.2 cerb_plugins: cerberusweb.core, config: text/url: label: Your n8n webhook url records: automation/ticketInteraction: fields: name: wgm.n8n.interaction extension_id: cerb.trigger.interaction.worker description: worker interaction to send ticket info to n8n script@raw: start: set: config@json: {{ cerb_workflow_config('wgm.integrations.n8n')|json_encode }} await/sending: duration: message: Sending to n8n... until: 1 second http.request/n8n: output: http_response inputs: method: GET url: {{ config.url }} body: record_id: {{ caller_params.record_id }} worker_id: {{ worker_id }} worker_mention: {{ worker_at_mention_name }} policy_kata@raw: commands: http.request: deny/method@bool: {{ inputs.method not in ['GET'] }} allow@bool: yes toolbar_section/n8nToolbar: fields: name: n8n toolbar_name: record.profile priority@int: 50 is_disabled: 0 toolbar_kata@raw: interaction/n8nPrompt: label: Send to n8n icon: electricity uri: cerb:automation:wgm.n8n.interaction hidden@bool: {{ record__type is not pattern ("ticket") }}
```
