...
123: {{ 123 is null ? 'null' : 'constant' }} {% set name = 'Kina Halpue' %}
...
```
{% set recipient = "support@cerb.example" %} {{ recipient is pattern ( "support@*" , "*@example.com" ) }}
```
...
```
{% set subject = "[Bugs] New issue reported" %} {{ subject is prefixed ( "[Bugs]" ) }}
```
...
```
{% set record__context = 'cerberusweb.contexts.task' %} {{ record__context is record type ( 'task' , 'ticket' ) }}
```
...
```
{% set number = 1 %} {% set string = "1" %} {{ number is same as string ? 'same' : 'not same' }}
```
...
```
{% set domain = "cerb.ai" %} {{ domain is suffixed ( ".ai" , ".com" ) }}
```
